...
get_emmeans <- function(
  ...
  assertthat::assert_that(is.character(specs_string), msg = "specs_string must be a character string")
  assertthat::assert_that(is.element(lmer.df, c("Asymptotic", "Satterthwaite", "Kenward-Roger")),
              msg = "lmer.df must be one of 'Asymptotic', 'Satterthwaite', or 'Kenward-Roger'")
  assertthat::assert_that(is.element(adjust, c("none", "tukey", "bonferroni")),
              msg = "adjust must be one of 'none', 'tukey', or 'bonferroni'")
  assertthat::assert_that(is.element(lang, c("eng", "ger")),
              msg = "lang must be one of 'eng' or 'ger'")
  assertthat::assert_that(is.element(sort, c("none", "asc", "desc")),
              msg = "sort must be one of 'none', 'asc', or 'desc'")
